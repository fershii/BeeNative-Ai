class Tetragonisca extends HTMLElement {
  connectedCallback() {
    this.innerHTML = `
    <style>
  ...
  <p class="text-center text-[#FAE45E] text-5xl font-semibold justify-center mb-4">Género Tetragonisca</p>
  ...
  <section class="bg-[#283B0A] flex items-center justify-center min-h-screen p-2">
    ...
    <div class="flex gap-6 w-full max-w-6xl h-[500px]">
      ...
      >
        <div>
          ...
          <p id="info-descripcion" class="info-field text-sm text-gray-700 leading-relaxed mb-3">
            Abeja pequeña de color amarillo dorado y negro. Es una de las especies más dóciles y apreciadas en la meliponicultura.
          </p>

          <div class="mt-3 space-y-2">
            <div>
              <span class="text-xs font-bold text-green-700 uppercase">Tamaño</span>
              <p id="info-tamaño" class="info-field text-sm text-gray-700">4-5 mm.</p>
            </div>
            <div>
              <span class="text-xs font-bold text-green-700 uppercase">Distribución</span>
              <p id="info-distribucion" class="info-field text-sm text-gray-700">Desde México hasta Argentina, incluyendo todo Centroamérica.</p>
            </div>
            <div>
              <span class="text-xs font-bold text-green-700 uppercase">Importancia</span>
              <p id="info-importancia" class="info-field text-sm text-gray-700">Excelente polinizadora y productora de una miel muy valorada por sus propiedades medicinales.</p>
            </div>
            <div>
              <span class="text-xs font-bold text-green-700 uppercase">Nivel de Manejo</span>
              <p id="info-nivelmanejo" class="info-field text-sm text-gray-700">Fácil, ideal para principiantes.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="panel-principal" class="w-1/2 h-full flex items-center justify-center">
        ...
        <div
          id="card-A"
          onclick="window.abrirImagen(this); window.tetragoniscaSeleccionar(this, event)"
          data-title="Tetragonisca angustula"
          data-desc="Abeja pequeña de color amarillo dorado y negro. Es una de las especies más dóciles y apreciadas en la meliponicultura."
          data-size="4-5 mm."
          data-distribution="Desde México hasta Argentina, incluyendo todo Centroamérica."
          data-importance="Excelente polinizadora y productora de una miel muy valorada por sus propiedades medicinales."
          data-management="Fácil, ideal para principiantes."
      ...
      <div id="columna-lateral" class="w-1/4 h-full flex flex-col gap-3">
        ...
        <div
          id="card-B"
          onclick="window.tetragoniscaSeleccionar(this, event)"
          data-title="-"
          data-desc="-"
          data-size="-"
          data-distribution="-"
    ...
    if (!window.tetragoniscaSeleccionar) {
      window.tetragoniscaSeleccionar = function(tarjetaClickeada, event) {
        // Encontrar el componente raíz al que pertenece la tarjeta clickeada
        const root = tarjetaClickeada.closest('tetragonisca-section');
        if (!root) return;
...
customElements.define('tetragonisca-section', Tetragonisca);